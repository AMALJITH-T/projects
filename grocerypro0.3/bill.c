...
void generate_bill();
	void calculate_bill();
 char *name[] = {"Brown_sugar", "buttermilk", "vegetable masala", "washing powder", "salt", "poha", "oil", "tooth paste", "hair conditioner", "wheat flour", "red chilli", "Jaggery", "coffee", "tea", "butter", "buttermilk powder", "turmeric powder", "hair oil", "ghee", "face powder"};
 char name_[19][20];
	int product_quantity[19];
int count1 = 0, total_bill = 0, total_bill1 = 0, discount;
int Brown_sugar_p = 50, buttermilk_p = 50, vegetable_masala_p = 50, washing_powder_p = 100, salt_p = 20, poha_p = 50, oil_p = 200, tooth_paste_p = 40, hair_conditioner_p = 120, wheat_flour_p = 50, red_chilli_p = 40, Jaggery_p = 45, coffee_p = 200, tea_p = 100, butter_p
= 100, buttermilk_powder_p = 150, turnmeric_powder_p = 200, hair_oil_p = 60, ghee_p = 300, face_powder_p = 70;
int main()
{
	char ch, temp_str[20];
	int check,count=0;
 ...
 
{
	...
	printf("| %s	\t%dpac	\t\t%d \n", name_[i], product_quantity[i], washing_powder_p*product_quantity[i]);
	printf("| 	-----\n"); }
	...
	printf("| 	\n");}
	else if (!strcmp("hair conditioner", name_[i])){
	printf("| %s	\t%dpac	\t\t%d \n", name_[i], product_quantity[i], hair_conditioner_p*product_quantity[i]);
	printf("| 	\n");}
...
product_quantity[i];
	...
	total_bill += hair_conditioner_p * product_quantity[i];
else if (!strcmp("wheat flour", name_[i]))
